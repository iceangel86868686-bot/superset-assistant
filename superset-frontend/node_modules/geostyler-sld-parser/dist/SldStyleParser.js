...
import { geoStylerFunctionToSldFunction, get, getAttribute, getBase64Object, getChildren, getParameterValue, getVendorOptionValue, isSymbolizer, keysByValue, numberExpression } from './Util/SldUtil';
...
const unitSldMetre = 'http://www.opengeospatial.org/se/units/metre';
const unitSldPixel = 'http://www.opengeospatial.org/se/units/pixel';
...
export class SldStyleParser {
     ...
     */
    getOffsetFromDisplacement(displacement) {
     ...
     */
    getPointSymbolizerFromSldSymbolizer(sldSymbolizer, distanceUnit = undefined) {
        const sldPointSymbolizer = sldSymbolizer.PointSymbolizer;
        let pointSymbolizer;
        const wellKnownName = get(sldPointSymbolizer, 'Graphic.Mark.WellKnownName.#text');
        const externalGraphic = get(sldPointSymbolizer, 'Graphic.ExternalGraphic');
        if (externalGraphic) {
    ...
    getLineSymbolizerFromSldSymbolizer(sldSymbolizer) {
        ...
        const distanceUnit = this.getDistanceUnit(sldSymbolizer);
        ...
        if (!isNil(placement)) {
            ...
            if (!isNil(pointPlacement)) {
                ...
                if (this.readingSldVersion === '1.1.0') {
                    ...
                    const isRepeated = get(linePlacement, 'IsRepeated.#text');
    ...
    getTextSymbolizerLabelFromSldSymbolizer = (sldLabel) => {
        if (Array.isArray(sldLabel) &&
            ...
            const convertSldArgToGeoStyler = (arg) => {
    ...
    getFillSymbolizerFromSldSymbolizer(sldSymbolizer) {
        ...
        if (!isNil(graphicFill)) {
            fillSymbolizer.graphicFill = this.getPointSymbolizerFromSldSymbolizer({ PointSymbolizer: graphicFill });
        }
    ...
    getMarkSymbolizerFromSldSymbolizer(sldSymbolizer) {
        ...
        const distanceUnit = this.getDistanceUnit(sldSymbolizer);
        const wellKnownName = get(sldMarkSymbolizer, 'Graphic.Mark.WellKnownName.#text');
     ...
     */
    getIconSymbolizerFromSldSymbolizer(sldSymbolizer, forcedDistanceUnit = undefined) {
        const sldIconSymbolizer = sldSymbolizer.PointSymbolizer;
        const distanceUnit = forcedDistanceUnit || this.getDistanceUnit(sldSymbolizer);
        ...
        const opacity = get(sldIconSymbolizer, 'Graphic.Opacity.#text');
        const size = get(sldIconSymbolizer, 'Graphic.Size.#text');
        const rotation = get(sldIconSymbolizer, 'Graphic.Rotation.#text');
     ...
     */
    geoStylerStyleToSldObject(geoStylerStyle) {
        const rules = this.getSldRulesFromRules(geoStylerStyle.rules);
    ...
    getSldComparisonFilterFromComparisonFilter(comparisonFilter) {
        const sldComparisonFilter = [];
        const operator = comparisonFilter[0];
        const key = comparisonFilter[1];
        const value = comparisonFilter[2];
        const sldOperators = keysByValue(COMPARISON_MAP, operator);
        const sldOperator = (sldOperators.length > 1 && value === null)
        ...
        if (sldOperator === 'PropertyIsNull') {
            ...
            const betweenFilter = comparisonFilter;
            sldComparisonFilter.push({
                [sldOperator]: [{
                        [propertyKey]: [{
                                '#text': key
    ...
    getSldFilterFromFilter(filter) {
        ...
        if (isComparisonFilter(filter)) {
            ...
            const sldOperators = keysByValue(COMBINATION_MAP, operator);
     ...
     */
    moveUomEntryToAttributes(sldSymbolizer, sldSymbolizerProperties) {
     ...
     */
    addUomEntry(sldSymbolizerProperties, unit) {
        if (unit === 'm') {
     ...
     */
    getSldSymbolizersFromSymbolizers(symbolizers) {
        const sldSymbolizers = [];
     ...
     */
    getDisplacementFromOffset(offset) {
     ...
     */
    getSldPointSymbolizerFromMarkSymbolizer(markSymbolizer) {
     ...
     */
    getSldPointSymbolizerFromIconSymbolizer(iconSymbolizer) {
        const Opacity = this.getTagName('Opacity');
        const Rotation = this.getTagName('Rotation');
        const Size = this.getTagName('Size');
        const Graphic = this.getTagName('Graphic');
        ...
        if (typeof iconSymbolizer.image === 'string' || iconSymbolizer.image instanceof String) {
            const base64Object = getBase64Object(iconSymbolizer.image);
            if (base64Object) {
                graphic.push(this.getSldExternalResourceInlineContent(base64Object));
            }
            else {
                graphic.push(this.getSldExternalResourceOnlineResource(iconSymbolizer.image));
            }
        }
     ...
     */
    getSldExternalResourceInlineContent(image) {
     ...
     */
    getSldExternalResourceOnlineResource(image) {
        const ExternalGraphic = this.getTagName('ExternalGraphic');
        const Format = this.getTagName('Format');
     ...
     */
    getSldAnchorPointFromAnchor(anchor, dimension) {
     ...
     */
    getAnchorFromSldAnchorPoint(anchorX, anchorY) {
        ...
        const gsAnchor = ((gsAnchorHoriz && gsAnchorVert) ?
            (gsAnchorVert + '-' + gsAnchorHoriz) : (gsAnchorVert + gsAnchorHoriz));
     ...
     */
    getSldTextSymbolizerFromTextSymbolizer(textSymbolizer) {
        const CssParameter = this.getTagName('CssParameter');
        const Fill = this.getTagName('Fill');
        ...
        const fontCssParameters = Object.keys(textSymbolizer)
            .filter((property) => property !== 'kind' && fontPropertyMap[property])
            .map((property) => {
     ...
     */
    getSldLabelFromTextSymbolizer = (template) => {
   ...
   */
    geoStylerFunctionToSldFunctionRecursive(func) {
        if (func === null) {
            return func;
        }
        // Literal at root
        ...
        if (typeof func === 'object') {
            ...
            if (typeof func.name === 'string' || Array.isArray(func.args)) {
                const sldArgs = func.args.map((arg) => {
                    // Property function
     ...
     */
    getSldLineSymbolizerFromLineSymbolizer(lineSymbolizer) {
        const CssParameter = this.getTagName('CssParameter');
        const Stroke = this.getTagName('Stroke');
        ...
        const sldLineSymbolizer = [];
        const cssParameters = Object.keys(lineSymbolizer)
            .filter((property) => property !== 'kind' && propertyMap[property] &&
     ...
     */
    getSldPolygonSymbolizerFromFillSymbolizer(fillSymbolizer) {
        const CssParameter = this.getTagName('CssParameter');
        const Stroke = this.getTagName('Stroke');
        const Fill = this.getTagName('Fill');
        ...
        Object.keys(fillSymbolizer)
            ...
            .filter((property) => fillSymbolizer[property] !== undefined &&
            ...
            if (Object.keys(fillPropertyMap).includes(property)) {
                ...
                if (property === 'outlineDasharray') {
                    const paramValue = fillSymbolizer[property];
                    transformedValue = '';
                    paramValue.forEach((dash, idx) => {
     ...
     */
    getSldRasterSymbolizerFromRasterSymbolizer(rasterSymbolizer) {
        const sldRasterSymbolizer = [];
     ...
     */
    getSldColorMapFromColorMap(colorMap) {
        const sldColorMap = [];
        // parse colorMap.type
        if (colorMap.type) {
        ...
        if (colorMap.colorMapEntries && colorMap.colorMapEntries.length > 0) {
            const colorMapEntries = colorMap.colorMapEntries.map((entry) => {
                const result = {
                    ColorMapEntry: [],
                    ':@': {}
                };
     ...
     */
    getSldChannelSelectionFromChannelSelection(channelSelection) {
        const propertyMap = {
    ...
    /**
       ...
       */
    getSldContrastEnhancementFromContrastEnhancement(contrastEnhancement) {
        const sldContrastEnhancement = [];
