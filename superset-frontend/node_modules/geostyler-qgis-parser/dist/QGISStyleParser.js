...
export class QGISStyleParser {
    ...
    qmlSymbolizerLayerPropsToObject(qmlSymbolizer) {
        ...
        if (qmlSymbolizer.Option && qmlSymbolizer.Option[0].Option) {
            qmlSymbolizer.Option[0].Option.forEach((option) => {
    ...
    getFilterFromQmlRule(qmlRule) {
        ...
        if (qmlFilter) {
            const f = this.cqlParser.read(qmlFilter);
            return f;
        }
        return undefined;
    }
    ...
    getLineSymbolizersFromQmlSymbolizer(qmlSymbolizer) {
        ...
        return qmlSymbolizer.layer.map((symbolizerLayer) => {
            ...
            if (!qmlMarkerProps.use_custom_dash || qmlMarkerProps.use_custom_dash === '0') {
                ...
                const lineDashArray = lineStyleDashArrays[lineStyle];
                if (lineDashArray) {
    ...
    getFillSymbolizerFromQmlSymbolizer(qmlSymbolizer) {
        const { qmlSymbolizerLayerPropsToObject } = this;
        return qmlSymbolizer.layer.flatMap((symbolizerLayer) => {
            let fillSymbolizer = {
                kind: 'Fill',
            };
            const qmlMarkerProps = qmlSymbolizerLayerPropsToObject(symbolizerLayer);
            ...
            if (symbolizerLayer.$.class === 'PointPatternFill') {
                const graphicFillList = symbolizerLayer.symbol.flatMap((x) => this.getPointSymbolizersFromQmlSymbolizer(x));
     ...
     */
    propsObjectToQmlSymbolOptions(properties) {
    ...
    propsObjectToQmlSymbol_NewerOptions(properties) {
    ...
    propsObjectToQmlSymbol_OlderProps(properties) {
