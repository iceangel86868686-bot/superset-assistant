class MapboxStyleUtil {
    ...
    static symbolizerAllUndefined(symbolizer) {
        return !Object.keys(symbolizer)
            .filter(val => val !== 'kind' && val !== 'visibility')
            .some((val) => typeof symbolizer[val] !== 'undefined');
    }
    /**
     ...
     * @param url URL
     */
    static getUrlForMbPlaceholder(url) {
     ...
     * @param url URL
     */
    static getMbPlaceholderForUrl(url) {
        const mbPlaceholder = 'mapbox://';
        const mbUrl = 'https://api.mapbox.com/';
     ...
     */
    static resolveMbTextPlaceholder(template) {
        ...
        const gsLabel = template.replace(regExp, (match) => {
            return `{${match}}`;
        });
        return gsLabel;
    }
    static getSpriteName(sprite, metadata) {
