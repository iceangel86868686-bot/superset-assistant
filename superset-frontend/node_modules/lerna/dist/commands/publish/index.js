...
function createTempLicenses(srcLicensePath, packagesToBeLicensed) {
  ...
  const licenseFileName = import_path19.default.basename(srcLicensePath);
  const options = {
  ...
  packagesToBeLicensed.forEach((pkg) => {
    pkg.licensePath = import_path19.default.join(pkg.contents, licenseFileName);
  });
...
var import_fs_extra8, import_p_map3, import_path19;
...
function getPackagesWithoutLicense(project, packagesToPublish) {
  return project.getPackageLicensePaths().then((licensePaths) => {
    const licensed = new Set(licensePaths.map((lp) => import_path20.default.dirname(lp)));
...
var import_devkit6, import_fs5, import_path21, import_slash2, childProcess15, resolvedPrettier;
...
async function updateLockfileVersion(pkg) {
  const lockfilePath = import_path22.default.join(pkg.location, "package-lock.json");
