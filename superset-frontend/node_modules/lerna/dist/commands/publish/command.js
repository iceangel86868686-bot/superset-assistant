...
function getCycles(dependencies) {
  ...
  function dfs(next, path20) {
    visited.add(next);
    path20.push(next);
    for (const dep of dependencies[next] || []) {
      if (path20.includes(dep)) {
        const cycle = path20.slice(path20.indexOf(dep));
        cycles.push(cycle);
...
var require_config_chain = __commonJS({
  "libs/core/src/lib/npm-conf/config-chain/index.ts"(exports2, module2) {
    ...
    var find2 = exports2.find = function() {
      var rel = path20.join.apply(null, [].slice.call(arguments));
      function find3(start, rel2) {
        var file = path20.join(start, rel2);
        try {
          fs15.statSync(file);
          return file;
        } catch (err) {
          if (path20.dirname(start) !== start)
            return find3(path20.dirname(start), rel2);
        }
      }
    ...
    var json = exports2.json = function() {
      ...
      var file = path20.join.apply(null, args);
      var content;
      try {
...
function printCommandBanner(id, event, cmd, path20) {
  return console.log(`
> ${id ? `${id} ` : ""}${event} ${path20}
> ${cmd.trim().replace(/\n/g, "\n> ")}
`);
...
var import_fs_extra6, import_path15, import_ssri, tar;
...
async function tempWrite(fileContent, filePath) {
  ...
  await import_node_fs.default.promises.mkdir(import_path16.default.dirname(tempPath), { recursive: true });
  await write(tempPath, fileContent);
  return tempPath;
}
var import_node_crypto, import_node_fs, import_node_os2, import_path16, tempDir, tempfile, writeStream, temp_write_default;
var init_temp_write = __esm({
  "libs/core/src/lib/temp-write.ts"() {
    ...
    tempfile = (filePath) => import_path16.default.join(tempDir, (0, import_node_crypto.randomUUID)(), filePath || "");
    ...
    tempWrite.sync = (fileContent, filePath) => {
      const tempPath = tempfile(filePath);
      import_node_fs.default.mkdirSync(import_path16.default.dirname(tempPath), { recursive: true });
...
var import_arborist, import_npm_packlist, import_path17, tar2;
...
function resolvePrettier() {
  if (!resolvedPrettier) {
    try {
      const packageJson = (0, import_devkit5.readJsonFile)(import_path19.default.join(import_devkit5.workspaceRoot, "package.json"));
...
async function maybeFormatFile(filePath) {
  ...
  const ignorePath = import_path19.default.join(import_devkit5.workspaceRoot, ".prettierignore");
  const fullFilePath = import_path19.default.join(import_devkit5.workspaceRoot, filePath);
...
async function gitAdd(changedFiles, gitOpts, execOpts) {
  ...
  for (const file of changedFiles) {
    const filePath = (0, import_slash2.default)(import_path19.default.relative(execOpts.cwd, import_path19.default.resolve(execOpts.cwd, file)));
...
var import_devkit5, import_fs5, import_path19, import_slash2, childProcess12, resolvedPrettier;
...
async function updateLockfileVersion(pkg) {
  const lockfilePath = import_path20.default.join(pkg.location, "package-lock.json");
...
var import_devkit6, import_load_json_file3, import_path20;
      ...
      async updatePackageVersions() {
        ...
        if (this.options.npmClient === "yarn") {
          ...
          if (import_semver8.default.gte(yarnVersion, "2.0.0")) {
            ...
            const lockfilePath = import_path23.default.join(this.project.rootPath, "yarn.lock");
            changedFiles.add(lockfilePath);
          }
        }
...
function createTempLicenses(srcLicensePath, packagesToBeLicensed) {
  ...
  const licenseFileName = import_path21.default.basename(srcLicensePath);
  const options = {
  ...
  packagesToBeLicensed.forEach((pkg) => {
    pkg.licensePath = import_path21.default.join(pkg.contents, licenseFileName);
  });
...
var import_fs_extra8, import_p_map3, import_path21;
...
function getPackagesWithoutLicense(project, packagesToPublish) {
  return project.getPackageLicensePaths().then((licensePaths) => {
    const licensed = new Set(licensePaths.map((lp) => import_path22.default.dirname(lp)));
...
function interpolate(template, data) {
  ...
  return res.replace(/{([\s\S]+?)}/g, (match) => {
    let value = data;
    const path20 = match.slice(1, -1).trim().split(".");
    for (let idx = 0; idx < path20.length; idx++) {
      if (!value[path20[idx]]) {
        return match;
      }
      value = value[path20[idx]];
    }
    return value;
  });
}
