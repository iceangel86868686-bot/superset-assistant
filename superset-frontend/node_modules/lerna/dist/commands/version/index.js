  ...
  "libs/core/src/lib/package.ts"() {
    ...
    Package = class _Package {
      ...
      constructor(pkg, location, rootPath = location) {
        const resolved = import_npm_package_arg.default.resolve(pkg.name, `file:${import_path3.default.relative(rootPath, location)}`, rootPath);
        this.name = pkg.name;
        this[PKG] = pkg;
...
async function createProjectGraphWithPackages(projectGraph, projectFileMap, packageConfigs) {
  ...
  sortedTuples.forEach(([node, manifest]) => {
    let pkg = null;
    if (manifest) {
      pkg = new Package(manifest, (0, import_path9.join)(_workspaceRoot, node.data.root), _workspaceRoot);
...
function readExistingChangelog(pkg) {
  const changelogFileLoc = import_path10.default.join(pkg.location, "CHANGELOG.md");
  let chain = Promise.resolve();
...
function getCycles(dependencies) {
  ...
  function dfs(next, path15) {
    visited.add(next);
    path15.push(next);
    for (const dep of dependencies[next] || []) {
      if (path15.includes(dep)) {
        const cycle = path15.slice(path15.indexOf(dep));
        cycles.push(cycle);
...
var require_types = __commonJS({
  "libs/core/src/lib/npm-conf/types.ts"(exports2) {
    ...
    exports2.types = {
      ...
      cafile: import_path18.default,
      cache: import_path18.default,
      ...
      global: Boolean,
      globalconfig: import_path18.default,
      ...
      "init-module": import_path18.default,
      ...
      tag: String,
      timing: Boolean,
      tmp: import_path18.default,
      unicode: Boolean,
      "unsafe-perm": Boolean,
      "update-notifier": Boolean,
      usage: Boolean,
      user: [Number, String],
      userconfig: import_path18.default,
      umask: Umask,
      version: Boolean,
...
var require_config_chain = __commonJS({
  "libs/core/src/lib/npm-conf/config-chain/index.ts"(exports2, module2) {
    ...
    var find2 = exports2.find = function() {
      var rel = path15.join.apply(null, [].slice.call(arguments));
      function find3(start, rel2) {
        var file = path15.join(start, rel2);
        try {
          fs13.statSync(file);
          return file;
        } catch (err) {
          if (path15.dirname(start) !== start)
            return find3(path15.dirname(start), rel2);
        }
      }
    ...
    var json = exports2.json = function() {
      ...
      var file = path15.join.apply(null, args);
      var content;
      try {
...
function npmInstallDependencies(pkg, dependencies, config) {
  ...
  return import_fs_extra4.default.copy(pkg.manifestLocation, packageJsonBkp).then(() => {
    ...
    return writePackage(pkg.manifestLocation, tempJson).then(() => npmInstall(pkg, config)).then(() => done(), done);
  });
}
...
var require_defaults = __commonJS({
  "libs/core/src/lib/npm-conf/defaults.ts"(exports2) {
    ...
    if (home) {
      ...
      home = import_path18.default.resolve(temp, `npm-${uidOrPid}`);
    }
    var cacheExtra = process.platform === "win32" ? "npm-cache" : ".npm";
    var cacheRoot = process.platform === "win32" && process.env["APPDATA"] || home;
    var cache = import_path18.default.resolve(cacheRoot, cacheExtra);
    var defaults;
    var globalPrefix;
    Object.defineProperty(exports2, "defaults", {
      get() {
        ...
        if (process.env["PREFIX"]) {
          ...
          globalPrefix = import_path18.default.dirname(process.execPath);
        } else {
          globalPrefix = import_path18.default.dirname(import_path18.default.dirname(process.execPath));
          if (process.env["DESTDIR"]) {
            globalPrefix = import_path18.default.join(process.env["DESTDIR"], globalPrefix);
          }
        }
        defaults = {
          access: null,
          ...
          "commit-hooks": true,
          global: false,
          globalconfig: import_path18.default.resolve(globalPrefix, "etc", "npmrc"),
          "global-style": false,
          ...
          "init-module": import_path18.default.resolve(home, ".npm-init.js"),
          "init-author-name": "",
          "init-author-email": "",
          "init-author-url": "",
          "init-version": "1.0.0",
          ...
          userconfig: import_path18.default.resolve(home, ".npmrc"),
...
var require_npm_conf = __commonJS({
  "libs/core/src/lib/npm-conf/index.ts"(exports2, module2) {
    ...
    function npmConf3(opts) {
      ...
      conf.addEnv();
      conf.loadPrefix();
      const projectConf = import_path18.default.resolve(conf["localPrefix"], ".npmrc");
      ...
      if (conf["get"]("prefix")) {
        const etc = import_path18.default.resolve(conf["get"]("prefix"), "etc");
        conf.root.globalconfig = import_path18.default.resolve(etc, "npmrc");
        conf.root.globalignorefile = import_path18.default.resolve(etc, "npmignore");
      }
...
function printCommandBanner(id, event, cmd, path15) {
  return console.log(`
> ${id ? `${id} ` : ""}${event} ${path15}
> ${cmd.trim().replace(/\n/g, "\n> ")}
`);
...
async function tempWrite(fileContent, filePath) {
  ...
  await import_node_fs.default.promises.mkdir(import_path14.default.dirname(tempPath), { recursive: true });
  await write(tempPath, fileContent);
  return tempPath;
}
var import_node_crypto, import_node_fs, import_node_os2, import_path14, tempDir, tempfile, writeStream, temp_write_default;
var init_temp_write = __esm({
  "libs/core/src/lib/temp-write.ts"() {
    ...
    tempDir = import_node_fs.default.realpathSync(import_node_os2.default.tmpdir());
    tempfile = (filePath) => import_path14.default.join(tempDir, (0, import_node_crypto.randomUUID)(), filePath || "");
    ...
    tempWrite.sync = (fileContent, filePath) => {
      const tempPath = tempfile(filePath);
      import_node_fs.default.mkdirSync(import_path14.default.dirname(tempPath), { recursive: true });
...
var import_make_fetch_happen2, import_path15, GitLabClient;
...
Request returned ${status} ${statusText}`);
      ...
      releasesUrl(namespace, project) {
        return new URL(
          `${this.baseUrl}/${import_path15.default.join("projects", encodeURIComponent(`${namespace}/${project}`), "releases")}`
        ).toString();
      }
    };
  }
...
function resolvePrettier() {
  if (!resolvedPrettier) {
    try {
      const packageJson = (0, import_devkit5.readJsonFile)(import_path16.default.join(import_devkit5.workspaceRoot, "package.json"));
...
async function maybeFormatFile(filePath) {
  ...
  const ignorePath = import_path16.default.join(import_devkit5.workspaceRoot, ".prettierignore");
  const fullFilePath = import_path16.default.join(import_devkit5.workspaceRoot, filePath);
...
async function gitAdd(changedFiles, gitOpts, execOpts) {
  ...
  for (const file of changedFiles) {
    const filePath = (0, import_slash2.default)(import_path16.default.relative(execOpts.cwd, import_path16.default.resolve(execOpts.cwd, file)));
...
var import_devkit5, import_fs5, import_path16, import_slash2, childProcess12, resolvedPrettier;
...
var import_devkit6, import_load_json_file3, import_path17;
    ...
    var VersionCommand = class extends Command {
      ...
      async updatePackageVersions() {
        ...
        if (this.options.npmClient === "pnpm") {
          ...
          const lockfilePath = import_path18.default.join(this.project.rootPath, "pnpm-lock.yaml");
          changedFiles.add(lockfilePath);
        }
        if (this.options.npmClient === "yarn") {
          ...
          if (import_semver8.default.gte(yarnVersion, "2.0.0")) {
            ...
            const lockfilePath = import_path18.default.join(this.project.rootPath, "yarn.lock");
            changedFiles.add(lockfilePath);
          }
        }
        if (this.options.npmClient === "npm" || !this.options.npmClient) {
          const lockfilePath = import_path18.default.join(this.project.rootPath, "package-lock.json");
