...
var import_node_events2, import_node_util, consoleControl, Logger, log, trackerConstructors, mixinLog, npmlog_default;
...
async function writePackage(filePath, data) {
  const resolvedPath = import_path2.default.basename(filePath) === "package.json" ? filePath : import_path2.default.join(filePath, "package.json");
  const normalized = normalize(data);
  let indent = "  ";
  try {
    const existing = await (0, import_promises.readFile)(resolvedPath, "utf8");
    indent = detectIndent(existing);
  } catch {
  }
  const json = JSON.stringify(normalized, null, indent) + "\n";
  await (0, import_promises.writeFile)(resolvedPath, json);
}
var import_promises, import_path2, dependencyKeys;
var init_write_package = __esm({
  "libs/core/src/lib/write-package.ts"() {
    "use strict";
...
var import_devkit, import_fs, import_load_json_file, import_npm_package_arg, import_path3, PKG, _location, _resolved, _rootPath, _scripts, _contents, Package;
    ...
    Package = class _Package {
      ...
      static lazy(ref, dir = ".") {
        if (typeof ref === "string") {
          const location = import_path3.default.resolve(import_path3.default.basename(ref) === "package.json" ? import_path3.default.dirname(ref) : ref);
          const manifest = import_load_json_file.default.sync(import_path3.default.join(location, "package.json"));
      ...
      constructor(pkg2, location, rootPath = location) {
        const resolved = import_npm_package_arg.default.resolve(pkg2.name, `file:${import_path3.default.relative(rootPath, location)}`, rootPath);
        this.name = pkg2.name;
        this[PKG] = pkg2;
      ...
      get binLocation() {
        return import_path3.default.join(this.location, "node_modules", ".bin");
      }
      get manifestLocation() {
        return import_path3.default.join(this.location, "package.json");
      }
      get nodeModulesLocation() {
        return import_path3.default.join(this.location, "node_modules");
      }
      ...
      get contents() {
        ...
        if (publishConfig && publishConfig.directory) {
          return import_path3.default.join(this.location, publishConfig.directory);
        }
      ...
      set contents(subDirectory) {
        ...
        this[_contents] = import_path3.default.join(this.location, subDirectory);
      }
      // "live" collections
      get dependencies() {
      ...
      async syncDistVersion(doSync) {
        if (doSync) {
          const distPkg = import_path3.default.join(this.contents, "package.json");
          if (distPkg !== this.manifestLocation && import_fs.default.existsSync(distPkg)) {
            ...
            await writePackage(distPkg, pkg2);
          }
        }
        return this;
      }
...
function makeFileFinder(rootPath, packageConfigs) {
  ...
  return (fileName, fileMapper, customGlobOpts) => {
    ...
    const promise = (0, import_p_map.default)(
      ...
      (globPath) => {
        let chain = (0, import_tinyglobby.glob)(import_path5.default.posix.join(globPath, fileName), options);
...
var import_tinyglobby, import_p_map, import_path5;
...
var import_devkit2, import_cosmiconfig, import_dedent2, import_fs2, import_glob_parent, import_tinyglobby2, import_js_yaml, import_load_json_file2, import_p_map2, import_path6, LICENSE_GLOB, Project, getPackages, getPackagesSync;
  ...
  "libs/core/src/lib/project/index.ts"() {
    ...
    Project = class _Project {
      ...
      constructor(cwd, options) {
        ...
        this.rootPath = import_path6.default.dirname(filepath);
      ...
      get packageParentDirs() {
        return this.packageConfigs.map((packagePattern) => (0, import_glob_parent.default)(packagePattern)).map((parentDir) => import_path6.default.resolve(this.rootPath, parentDir));
      }
      get licensePath() {
        let licensePath;
        try {
...
var import_os, import_path7, import_write_file_atomic;
...
function getPackageManifestPath(node, files) {
  const pkgJsonPath = (0, import_path8.resolve)((0, import_path8.join)(node.data.root, "package.json"));
  return files.find((f) => (0, import_path8.resolve)(f.file) === pkgJsonPath)?.file;
}
var import_path8;
...
async function createProjectGraphWithPackages(projectGraph, projectFileMap, packageConfigs) {
  ...
  const tuples = await Promise.all(
    projectNodesMatchingPackageConfigs.map(
      (node) => new Promise((resolve3) => {
        ...
        if (manifestPath) {
          const fullManifestPath = (0, import_path9.join)(_workspaceRoot, manifestPath);
  ...
  sortedTuples.forEach(([node, manifest]) => {
    let pkg2 = null;
    if (manifest) {
      pkg2 = new Package(manifest, (0, import_path9.join)(_workspaceRoot, node.data.root), _workspaceRoot);
...
var import_devkit3, import_fs_extra, import_minimatch2, import_npm_package_arg2, import_path9, import_semver2, resolvePackage;
...
function normalizeNewPresetConfig(config) {
  if (config && (config.parser || config.writer) && !config.parserOpts && !config.writerOpts && !config.conventionalChangelog) {
    npmlog_default.verbose("getChangelogConfig", "Normalizing new preset API to legacy format");
...
async function resolveConfigPromise(presetPackageName, presetConfig) {
  ...
  if (isFunction(config)) {
    try {
      ...
      config = (0, import_node_util3.promisify)(config)();
    }
  }
...
var import_npm_package_arg3, import_node_util3, cfgCache;
...
function readExistingChangelog(pkg2) {
  const changelogFileLoc = import_path10.default.join(pkg2.location, "CHANGELOG.md");
  let chain = Promise.resolve();
...
var import_fs_extra2, import_path10;
...
function multimatch(list3, patterns, options = {}) {
  ...
  for (const item of list3) {
    for (let pattern of patterns) {
      ...
      result = process3(result, import_minimatch3.default.match([item], pattern, options));
    }
  }
  return result;
}
...
function formatColumns(resultList, viewOptions) {
  const formattedResults = resultList.map((project) => {
    ...
    formatted.location = import_chalk3.default.grey(import_path11.default.relative(".", pkg2.location));
    return formatted;
  });
  return trimmedColumns(formattedResults, viewOptions);
}
var import_chalk3, import_columnify, import_path11;
...
function findPrefix(start) {
  let dir = import_path12.default.resolve(start);
  let walkedUp = false;
  while (import_path12.default.basename(dir) === "node_modules") {
...
function find(name, original) {
  ...
  try {
    ...
    const dirname = import_path12.default.dirname(name);
    if (dirname === name) {
      return original;
    }
...
var require_types = __commonJS({
  "libs/core/src/lib/npm-conf/types.ts"(exports2) {
    ...
    exports2.types = {
      ...
      cafile: import_path26.default,
      cache: import_path26.default,
      ...
      global: Boolean,
      globalconfig: import_path26.default,
      ...
      "init-module": import_path26.default,
      ...
      tag: String,
      timing: Boolean,
      tmp: import_path26.default,
      unicode: Boolean,
      "unsafe-perm": Boolean,
      "update-notifier": Boolean,
      usage: Boolean,
      user: [Number, String],
      userconfig: import_path26.default,
      umask: Umask,
      version: Boolean,
...
function parseField(input, key) {
  ...
  const isPath = typeList.indexOf(import_path13.default) !== -1;
  ...
  if (isPath) {
    ...
    if (regex.test(field) && process.env["HOME"]) {
      field = import_path13.default.resolve(process.env["HOME"], field.substr(2));
    }
    field = import_path13.default.resolve(field);
  }
...
var require_config_chain = __commonJS({
  "libs/core/src/lib/npm-conf/config-chain/index.ts"(exports2, module2) {
    ...
    var find2 = exports2.find = function() {
      var rel = path23.join.apply(null, [].slice.call(arguments));
      function find3(start, rel2) {
        var file = path23.join(start, rel2);
        try {
          fs16.statSync(file);
          return file;
        } catch (err) {
          if (path23.dirname(start) !== start)
            return find3(path23.dirname(start), rel2);
        }
      }
    ...
    var json = exports2.json = function() {
      ...
      var file = path23.join.apply(null, args);
      var content;
      try {
...
var import_assert, import_fs4, import_path14, ConfigChain, Conf;
var init_conf = __esm({
  "libs/core/src/lib/npm-conf/conf.ts"() {
    "use strict";
    ...
    Conf = class extends ConfigChain {
      ...
      loadPrefix() {
        ...
        Object.defineProperty(this, "globalPrefix", {
          ...
          get: () => import_path14.default.resolve(this["get"]("prefix"))
        });
        let p;
        ...
        if (Object.prototype.hasOwnProperty.call(cli, "prefix")) {
          p = import_path14.default.resolve(cli.prefix);
        } else {
          try {
      ...
      loadUser() {
        ...
        const prefix = import_path14.default.resolve(this["get"]("prefix"));
        try {
...
function npmInstallDependencies(pkg2, dependencies, config) {
  ...
  return import_fs_extra4.default.copy(pkg2.manifestLocation, packageJsonBkp).then(() => {
    ...
    return writePackage(pkg2.manifestLocation, tempJson).then(() => npmInstall(pkg2, config)).then(() => done(), done);
  });
}
...
var import_fs_extra4, import_npm_package_arg4, import_signal_exit2, childProcess8;
...
var require_defaults = __commonJS({
  "libs/core/src/lib/npm-conf/defaults.ts"(exports2) {
    ...
    if (home) {
      ...
      home = import_path26.default.resolve(temp, `npm-${uidOrPid}`);
    }
    var cacheExtra = process.platform === "win32" ? "npm-cache" : ".npm";
    var cacheRoot = process.platform === "win32" && process.env["APPDATA"] || home;
    var cache = import_path26.default.resolve(cacheRoot, cacheExtra);
    var defaults;
    var globalPrefix;
    Object.defineProperty(exports2, "defaults", {
      get() {
        ...
        if (process.env["PREFIX"]) {
          ...
          globalPrefix = import_path26.default.dirname(process.execPath);
        } else {
          globalPrefix = import_path26.default.dirname(import_path26.default.dirname(process.execPath));
          if (process.env["DESTDIR"]) {
            globalPrefix = import_path26.default.join(process.env["DESTDIR"], globalPrefix);
          }
        }
        defaults = {
          access: null,
          ...
          "commit-hooks": true,
          global: false,
          globalconfig: import_path26.default.resolve(globalPrefix, "etc", "npmrc"),
          "global-style": false,
          ...
          "init-module": import_path26.default.resolve(home, ".npm-init.js"),
          "init-author-name": "",
          "init-author-email": "",
          "init-author-url": "",
          "init-version": "1.0.0",
          ...
          userconfig: import_path26.default.resolve(home, ".npmrc"),
          // TODO: refactor based on TS feedback
          // eslint-disable-next-line @typescript-eslint/ban-ts-comment
          // @ts-ignore
...
var require_npm_conf = __commonJS({
  "libs/core/src/lib/npm-conf/index.ts"(exports2, module2) {
    ...
    function npmConf3(opts) {
      ...
      conf.addEnv();
      conf.loadPrefix();
      const projectConf = import_path26.default.resolve(conf["localPrefix"], ".npmrc");
      ...
      if (conf["get"]("prefix")) {
        const etc = import_path26.default.resolve(conf["get"]("prefix"), "etc");
        conf.root.globalconfig = import_path26.default.resolve(etc, "npmrc");
        conf.root.globalignorefile = import_path26.default.resolve(etc, "npmignore");
      }
...
async function npmPublish(pkg2, tarFilePath, options, conf, otpCache) {
  ...
  if (!dryRun) {
    ...
    if (pkg2.contents !== pkg2.location) {
      manifestLocation = import_path15.default.join(pkg2.contents, "package.json");
    }
    const [tarData, npmCliPackageJson] = await Promise.all([
      import_fs_extra5.default.readFile(tarFilePath),
      await (0, import_package_json.prepare)(import_path15.default.dirname(manifestLocation))
    ]);
...
var import_package_json, import_fs_extra5, import_libnpmpublish, import_npm_package_arg6, import_path15;
...
var import_fs_extra6, import_path16, import_ssri, tar;
...
async function tempWrite(fileContent, filePath) {
  ...
  const write = fileContent !== null && typeof fileContent === "object" && typeof fileContent.pipe === "function" ? writeStream : import_node_fs.default.promises.writeFile;
  await import_node_fs.default.promises.mkdir(import_path17.default.dirname(tempPath), { recursive: true });
  await write(tempPath, fileContent);
  return tempPath;
}
var import_node_crypto, import_node_fs, import_node_os3, import_path17, tempDir, tempfile, writeStream, temp_write_default;
var init_temp_write = __esm({
  "libs/core/src/lib/temp-write.ts"() {
    ...
    tempDir = import_node_fs.default.realpathSync(import_node_os3.default.tmpdir());
    tempfile = (filePath) => import_path17.default.join(tempDir, (0, import_node_crypto.randomUUID)(), filePath || "");
    ...
    tempWrite.sync = (fileContent, filePath) => {
      const tempPath = tempfile(filePath);
      import_node_fs.default.mkdirSync(import_path17.default.dirname(tempPath), { recursive: true });
...
async function packDirectory(_pkg, dir, options) {
  ...
  opts.log.verbose("pack-directory", import_path18.default.relative(".", pkg2.contents));
...
var import_arborist, import_npm_packlist, import_path18, tar2;
...
var import_node_fs2, import_promises2;
...
var import_make_fetch_happen2, import_path19, GitLabClient;
  ...
  "libs/commands/clean/src/index.ts"(exports2, module2) {
    ...
    var CleanCommand = class extends Command {
      ...
      initialize() {
        ...
        this.logger.info(
          "clean",
          this.directoriesToDelete.map((dir) => import_path26.default.relative(this.project.rootPath, dir)).join("\n")
        );
...
var require_builtin_npmrc = __commonJS({
  "libs/commands/create/src/lib/builtin-npmrc.ts"(exports2, module2) {
    ...
    function builtinNpmrc() {
      let resolvedPath = "";
      try {
        resolvedPath = import_path26.default.resolve(
          import_fs_extra13.default.realpathSync(import_path26.default.join(import_path26.default.dirname(process.execPath), "npm")),
          "../../npmrc"
        );
      } catch (err) {
      }
...
var require_cat_file = __commonJS({
  "libs/commands/create/src/lib/cat-file.ts"(exports2, module2) {
    ...
    function catFile(baseDir, fileName, content, opts = "utf8") {
      return import_fs_extra13.default.writeFile(import_path26.default.join(baseDir, fileName), `${content}
`, opts);
    }
  }
});
...
var require_src2 = __commonJS({
  "libs/commands/create/src/index.ts"(exports2, module2) {
    ...
    var import_pacote2 = __toESM(require("pacote"));
    var import_path26 = __toESM(require("path"));
    ...
    var LERNA_MODULE_DATA = require.resolve(import_path26.default.join(__dirname, "./lib/lerna-module-data.js"));
    ...
    var CreateCommand = class extends Command {
      initialize() {
        ...
        this.targetDir = import_path26.default.resolve(this.pkgsDir, this.dirName);
        this.binDir = import_path26.default.join(this.targetDir, "bin");
        this.binFileName = bin === true ? this.dirName : bin;
        this.libDir = import_path26.default.join(this.targetDir, esModule ? "src" : "lib");
        this.libFileName = `${this.dirName}.js`;
        this.testDir = import_path26.default.join(this.targetDir, "__tests__");
      ...
      _getPackagesDir(pkgLocation) {
        ...
        const normalizedPkgLocation = import_path26.default.resolve(this.project.rootPath, import_path26.default.normalize(pkgLocation)).toLowerCase();
      ...
      async execute() {
        await import_fs_extra13.default.mkdirp(this.libDir);
        await import_fs_extra13.default.mkdirp(this.testDir);
        await Promise.all([this.writeReadme(), this.writeLibFile(), this.writeTestFile()]);
        if (this.binFileName) {
          await import_fs_extra13.default.mkdirp(this.binDir);
        ...
        if (this.options.esModule) {
          this.logger.notice(
            ...
            import_dedent6.default`
              Ensure '${import_path26.default.relative(".", this.pkgsDir)}/*/${this.outDir}' has been added to ./.gitignore
        ...
        this.logger.success(
          "create",
          `New package ${data.name} created at ./${import_path26.default.relative(".", this.targetDir)}`
        );
      }
      gitConfig(prop) {
      ...
      resolveRelative(depNode) {
        const relPath = import_path26.default.relative(this.targetDir, depNode.package.location);
        const spec = import_npm_package_arg9.default.resolve(depNode.package.name, relPath, this.targetDir);
      ...
      setDependencies() {
        ...
        const decideVersion = (spec) => {
          ...
          if (spec.type === "tag" && spec.fetchSpec === "latest" || spec.type === "range" && spec.fetchSpec === "*") {
            ...
            return import_pacote2.default.manifest(spec, pacoteOpts).then((pkg2) => `${savePrefix}${pkg2.version}`);
          }
          if (spec.type === "git") {
      ...
      setHomepage() {
        ...
        const relativeTarget = import_path26.default.relative(this.project.rootPath, this.targetDir);
        if (hurl.hostname.match("github")) {
          hurl.pathname = import_path26.default.posix.join(hurl.pathname, "tree/main", relativeTarget);
          hurl.hash = "readme";
        } else if (!this.options.homepage) {
          hurl.pathname = import_path26.default.posix.join(hurl.pathname, relativeTarget);
        }
...
var require_command4 = __commonJS({
  "libs/commands/diff/src/command.ts"(exports2, module2) {
    ...
    var command = {
      ...
      async handler(argv) {
        return (await Promise.resolve().then(() => (init_src4(), src_exports3))).factory(argv);
      }
    };
    module2.exports = command;
  }
  ...
  "libs/commands/exec/src/index.ts"() {
    ...
    ExecCommand = class extends Command {
      ...
      runCommandInPackageStreaming(pkg2) {
        return childProcess11.spawnStreaming(this.command, this.args, this.getOpts(pkg2), this.prefix && pkg2.name);
      }
      runCommandInPackageCapturing(pkg2) {
        return childProcess11.spawn(this.command, this.args, this.getOpts(pkg2));
      }
    };
  }
});
...
var import_dedent5, import_fs_extra8, import_p_map_series, import_path20, childProcess12, ImportCommand;
var init_src6 = __esm({
  "libs/commands/import/src/index.ts"() {
    "use strict";
    init_src2();
    ...
    ImportCommand = class extends Command {
      ...
      initialize() {
        ...
        const externalRepoPath = import_path20.default.resolve(inputPath ?? "");
        const externalRepoBase = import_path20.default.basename(externalRepoPath);
        ...
        const packageJson = import_path20.default.join(externalRepoPath, "package.json");
        ...
        const targetDir = import_path20.default.join(targetBase, externalRepoBase);
        const gitRepoRoot = this.getWorkspaceRoot();
        const lernaRootRelativeToGitRoot = import_path20.default.relative(gitRepoRoot, this.project.rootPath);
        this.targetDirRelativeToGitRoot = import_path20.default.join(lernaRootRelativeToGitRoot, targetDir);
        ...
        if (import_fs_extra8.default.existsSync(import_path20.default.resolve(this.project.rootPath, targetDir))) {
      ...
      getPackageDirectories() {
        return this.project.packageConfigs.filter((p) => p.endsWith("*")).map((p) => import_path20.default.dirname(p));
      }
      getTargetBase() {
      ...
      execSync(cmd, args) {
        return childProcess12.execSync(cmd, args, this.execOpts);
      }
      externalExecSync(cmd, args) {
        return childProcess12.execSync(cmd, args, this.externalExecOpts);
      }
...
${version}`;
      ...
      execute() {
        ...
        const mapper = (sha) => {
          ...
          const proc = childProcess12.exec("git", procArgs, this.execOpts);
          ...
          return pulseTillDone(proc).then(() => {
            ...
            if (diff === "") {
              tracker.completeWork(1);
              return childProcess12.exec("git", ["am", "--skip"], this.execOpts);
            }
            err.sha = sha;
            throw err;
          });
...
var require_command6 = __commonJS({
  "libs/commands/import/src/command.ts"(exports2, module2) {
    ...
    var command = {
      ...
      async handler(argv) {
        return (await Promise.resolve().then(() => (init_src6(), src_exports4))).factory(argv);
      }
    };
    module2.exports = command;
  }
...
var require_command7 = __commonJS({
  "libs/commands/info/src/command.ts"(exports2, module2) {
    ...
    var command = {
      ...
      async handler(argv) {
        return (await Promise.resolve().then(() => (init_src7(), src_exports5))).factory(argv);
      }
    };
    module2.exports = command;
  }
...
var import_devkit5, import_fs5, import_fs_extra9, import_tree, LARGE_BUFFER, childProcess13, PACKAGE_GLOB, InitCommand;
var init_src8 = __esm({
  "libs/commands/init/src/index.ts"() {
    "use strict";
    init_src2();
    ...
    import_fs_extra9 = require("fs-extra");
      ...
      async execute() {
        ...
        changes.forEach((f) => {
          if (f.type === "CREATE") {
            ...
            if (isDryRun) {
              const currentContentsOnDisk = (0, import_fs_extra9.readFileSync)((0, import_devkit5.joinPathFragments)(tree.root, f.path)).toString();
...
`);
        ...
        return async () => {
          if (isGitInitialized(this.cwd)) {
            ...
            await childProcess13.exec("git", ["init"], {
              cwd: this.cwd,
              maxBuffer: 1024
            });
          }
          if (this.args.skipInstall === void 0) {
            ...
            await childProcess13.exec(command, args, {
              cwd: this.cwd,
...
var require_command8 = __commonJS({
  "libs/commands/init/src/command.ts"(exports2, module2) {
    ...
    var command = {
      ...
      async handler(argv) {
        return (await Promise.resolve().then(() => (init_src8(), src_exports6))).factory(argv);
      }
    };
    module2.exports = command;
  }
...
var require_command9 = __commonJS({
  "libs/commands/list/src/command.ts"(exports2, module2) {
    ...
    var command = {
      ...
      async handler(argv) {
        return (await Promise.resolve().then(() => (init_src9(), src_exports7))).factory(argv);
      }
    };
    module2.exports = command;
  }
...
function resolvePrettier() {
  if (!resolvedPrettier) {
    try {
      const packageJson = (0, import_devkit6.readJsonFile)(import_path21.default.join(import_devkit6.workspaceRoot, "package.json"));
...
async function maybeFormatFile(filePath) {
  ...
  const ignorePath = import_path21.default.join(import_devkit6.workspaceRoot, ".prettierignore");
  const fullFilePath = import_path21.default.join(import_devkit6.workspaceRoot, filePath);
...
async function gitAdd(changedFiles, gitOpts, execOpts) {
  ...
  for (const file of changedFiles) {
    const filePath = (0, import_slash2.default)(import_path21.default.relative(execOpts.cwd, import_path21.default.resolve(execOpts.cwd, file)));
...
var import_devkit6, import_fs6, import_path21, import_slash2, childProcess15, resolvedPrettier;
...
var import_devkit7, import_load_json_file3, import_path22;
...
var require_src3 = __commonJS({
  "libs/commands/version/src/index.ts"(exports2, module2) {
    ...
    var childProcess25 = (init_src(), __toCommonJS(src_exports));
    ...
    var VersionCommand = class extends Command {
      ...
      setGlobalVersionFloor() {
        ...
        for (const node of this.updates) {
          const pkg2 = getPackage(node);
          if (import_semver8.default.lt(pkg2.version, globalVersion)) {
            this.logger.verbose(
              "version",
      ...
      async updatePackageVersions() {
        ...
        if (this.options.npmClient === "pnpm") {
          ...
          const lockfilePath = import_path26.default.join(this.project.rootPath, "pnpm-lock.yaml");
          changedFiles.add(lockfilePath);
        }
        if (this.options.npmClient === "yarn") {
          ...
          if (import_semver8.default.gte(yarnVersion, "2.0.0")) {
            ...
            const lockfilePath = import_path26.default.join(this.project.rootPath, "yarn.lock");
            changedFiles.add(lockfilePath);
          }
        }
        if (this.options.npmClient === "npm" || !this.options.npmClient) {
          const lockfilePath = import_path26.default.join(this.project.rootPath, "package-lock.json");
          if (import_fs7.default.existsSync(lockfilePath)) {
            this.logger.verbose("version", "Updating root package-lock.json");
            await childProcess25.exec(
              "npm",
              [
                "install",
                "--package-lock-only",
...
function createTempLicenses(srcLicensePath, packagesToBeLicensed) {
  ...
  return (0, import_p_map4.default)(packagesToBeLicensed, (pkg2) => import_fs_extra10.default.copy(srcLicensePath, pkg2.licensePath, options));
}
var import_fs_extra10, import_p_map4, import_path23;
...
function getCurrentSHA(opts) {
  ...
  const sha = childProcess22.execSync("git", ["rev-parse", "HEAD"], opts);
...
var import_npm_package_arg8, childProcess23;
...
async function getProjectsWithUnpublishedPackages(projectNodes, opts) {
  ...
  const results = await (0, import_p_map5.default)(projectNodes, mapper, { concurrency: 4 });
  return results.filter(Boolean);
}
var import_p_map5, import_pacote;
...
function interpolate(template, data) {
  ...
  return res.replace(/{([\s\S]+?)}/g, (match) => {
    let value = data;
    const path23 = match.slice(1, -1).trim().split(".");
    for (let idx = 0; idx < path23.length; idx++) {
      if (!value[path23[idx]]) {
        return match;
      }
      value = value[path23[idx]];
    }
    return value;
  });
}
...
function removeTempLicenses(packagesToBeLicensed) {
  ...
  return (0, import_p_map6.default)(packagesToBeLicensed, (pkg2) => import_fs_extra11.default.remove(pkg2.licensePath));
}
var import_fs_extra11, import_p_map6;
...
var require_src4 = __commonJS({
  "libs/commands/publish/src/index.ts"(exports2, module2) {
    ...
    var versionCommand = require_src3();
    module2.exports = function factory9(argv) {
    ...
    var PublishCommand = class extends Command {
      ...
      configureProperties() {
        ...
        this.npmSession = import_node_crypto2.default.randomBytes(8).toString("hex");
      ...
      async execute() {
        ...
        if (this.options.summaryFile !== void 0) {
          ...
          output(jsonObject);
          try {
            import_node_fs3.default.writeFileSync(filePath, JSON.stringify(jsonObject));
          ...
          output(message.join(import_node_os4.default.EOL));
        }
      ...
      async prepareLicenseActions() {
        ...
        if (packagesWithoutLicense.length && !this.project.licensePath) {
          ...
          const list3 = names.length > 1 ? `${names.slice(0, -1).join(", ")}${names.length > 2 ? "," : ""} and ${names[names.length - 1]}` : names[0];
          this.logger.warn(
            "ENOLICENSE",
      ...
      async copyAssets(pkg2) {
        ...
        for (const file of filesToCopy) {
          if ((0, import_node_path.normalize)(file.from) === (0, import_node_path.normalize)(file.to)) {
          ...
          } else if ((0, import_node_fs3.existsSync)(file.from)) {
            this.logger.verbose(
              "publish",
            ...
            await (0, import_fs_extra13.copy)(file.from, file.to);
          } else {
            this.logger.warn(
              "EPUBLISHASSET",
...
var require_command11 = __commonJS({
  "libs/commands/publish/src/command.ts"(exports2, module2) {
    "use strict";
    init_src2();
    var versionCommand = require_command10();
...
var import_fs_extra12, import_run_many, import_run_one, import_p_map7, import_path25, import_perf_hooks, RunCommand;
var init_src10 = __esm({
  "libs/commands/run/src/index.ts"() {
    "use strict";
    init_src2();
    import_fs_extra12 = require("fs-extra");
    ...
    RunCommand = class extends Command {
      ...
      runScriptInPackagesParallel() {
        return (0, import_p_map7.default)(this.projectsWithScript, (p) => this.runScriptInPackageStreaming(getPackage(p)));
      }
      runScriptInPackagesLexical() {
        return (0, import_p_map7.default)(this.projectsWithScript, (p) => this.getRunner()(getPackage(p)), {
          concurrency: this.concurrency
        });
      }
      ...
      async prepNxOptions() {
        const nxJsonExists = (0, import_fs_extra12.existsSync)(import_path25.default.join(this.project.rootPath, "nx.json"));
  ...
  "packages/lerna/src/commands/add-caching/index.ts"(exports2, module2) {
    ...
    var AddCachingCommand = class extends Command {
      ...
      initialize() {
        ...
        const packages = Object.values(this.projectGraph.nodes).filter((p) => !!p.package).map((p) => p.package);
...
`
      ...
      async configureGitIgnore() {
        try {
          await (0, import_execa3.default)("git", ["check-ignore", ".nx/cache"]);
        } catch (e) {
          try {
            await (0, import_fs_extra13.appendFile)((0, import_devkit9.joinPathFragments)(import_devkit9.workspaceRoot, ".gitignore"), "\n.nx/cache\n");
          } catch (e2) {
            this.logger.warn(
              "add-caching",
...
var import_command3 = __toESM(require_command2());
var import_command4 = __toESM(require_command3());
var import_command5 = __toESM(require_command4());
var import_command6 = __toESM(require_command5());
var import_command7 = __toESM(require_command6());
var import_command8 = __toESM(require_command7());
var import_command9 = __toESM(require_command8());
var import_command10 = __toESM(require_command9());
var import_command11 = __toESM(require_command11());
var import_command12 = __toESM(require_command12());
var import_command13 = __toESM(require_command10());
var import_command14 = __toESM(require_command13());
var import_command15 = __toESM(require_command14());
var import_command16 = __toESM(require_command15());
process.env["NX_ISOLATE_PLUGINS"] = "false";
process.env["NX_TUI"] = "false";
...
module.exports = function main(argv) {
  ...
  const cli = lernaCLI().command(import_command14.default).command(import_command2.default).command(import_command3.default).command(import_command4.default).command(import_command5.default).command(import_command6.default).command(import_command7.default).command(import_command8.default).command(import_command9.default).command(import_command10.default).command(import_command11.default).command(import_command15.default).command(import_command12.default).command(import_command16.default).command(import_command13.default);
