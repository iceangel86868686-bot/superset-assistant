...
# Скрипт настройки Nginx для Superset на ALT Linux

echo "Начало настройки Nginx для Superset на ALT Linux..."

# Шаг 1. Создание директорий для конфигурации сайтов (если не существуют)
sudo mkdir -p /etc/nginx/sites-available.d
sudo mkdir -p /etc/nginx/sites-enabled.d

# Шаг 2. Создание конфигурационного файла
CONFIG_FILE="/etc/nginx/sites-available.d/superset.conf"
...
SYMLINK="/etc/nginx/sites-enabled.d/superset.conf"
...
DEFAULT_AVAILABLE="/etc/nginx/sites-available.d/default.conf"
DEFAULT_ENABLED="/etc/nginx/sites-enabled.d/default.conf"

if [ -f "$DEFAULT_AVAILABLE" ]; then
    echo "Удаление стандартного конфига из sites-available.d: $DEFAULT_AVAILABLE"
    sudo rm -f "$DEFAULT_AVAILABLE"
fi

if [ -f "$DEFAULT_ENABLED" ]; then
    echo "Удаление стандартного конфига из sites-enabled.d: $DEFAULT_ENABLED"
